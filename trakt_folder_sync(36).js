...
// === FIX: drop show in trakt (WORKING) ===
if (id && card && card.ids && card.ids.trakt) {
    var trakt_id = card.ids.trakt;

    log('Drop show in Trakt', trakt_id);

    try {
        // вытаскиваем requestApi через addToHistory
        var fn = window.TraktTV.api.addToHistory.toString();

        var requestApi = window.requestApi || (function(){
            try {
                return eval('requestApi');
            } catch(e){
                return null;
            }
        })();
        ...
        if (requestApi) {
            requestApi('POST', '/users/hidden/dropped', {
                shows: [{ ids: { trakt: trakt_id } }]
            }).then(function(){
                log('drop success', trakt_id);
            }).catch(function(e){
                log('drop failed', e);
            });
        } else {
            log('requestApi not found');
        }
