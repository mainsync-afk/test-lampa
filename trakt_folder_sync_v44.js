...
(function () {
    ...
    // -----------------------------------------------------------------------
    // Словарь tmdbId → traktId
    // Lampa хранит в favorite только массив ID, объекты карточек не сохраняются,
    // поэтому trakt ID нужно хранить отдельно
    // -----------------------------------------------------------------------
    ...
    function getTraktId(card) {
        ...
        if (!tmdb) return null;
        var map = Lampa.Storage.get(STORAGE_TRAKT_ID_MAP) || {};
        return map[tmdb] || null;
    }

    // -----------------------------------------------------------------------
    // Prямой вызов Trakt API через прокси
    // -----------------------------------------------------------------------
    ...
    function traktApiRequest(method, endpoint, body) {
        ...
        return fetch(TRAKT_PROXY + endpoint, opts).then(function (r) {
    ...
    function dropShowFromUpnext(card) {
        ...
        if (!tid) {
            log('dropShowFromUpnext: no trakt id', { tmdb: tmdbId(card) });
            return;
        }
        log('dropShowFromUpnext', { traktId: tid, title: card.title || card.name });
        traktApiRequest('POST', '/users/hidden/dropped', {
            shows: [{ ids: { trakt: parseInt(tid, 10) } }]
        })
    ...
    function applySync(folder, traktItems) {
        ...
        var toAdd = traktItems.filter(function (c) {
            ...
            // Для look: пропускаем если сериал уже в thrown
            // (upnext в Trakt может не сразу обновиться после hidden/dropped)
            if (folder === 'look') {
                var thrownIds = new Set((Lampa.Favorite.get({ type: 'thrown' }) || []).map(tmdbId).filter(Boolean));
                if (thrownIds.has(id)) {
        ...
        localItems.forEach(function (card) {
            ...
            try {
                _ownOps.add(id);
                // Для look: вместо удаления меняем статус
                // Ended/Canceled → viewed, всё остальное → continued
                if (folder === 'look') {
                    var endedStatuses = ['Ended', 'Canceled'];
                    var newStatus = endedStatuses.indexOf(card.status) >= 0 ? 'viewed' : 'continued';
                    log('look ended - changing status', { id: id, title: card.title || card.name, tmdbStatus: card.status, newStatus: newStatus });
                    // Явно снимаем look — id уже в _ownOps, поэтому
                    // remove listener не вызовет dropShowFromUpnext
                    Lampa.Favorite.remove('look', card);
                    Lampa.Favorite.add(newStatus, card);
                } else {
    ...
    function enrichAndAdd(folder, card, onDone) {
        ...
        network.silent(url, function (data) {
            ...
            // Сохраняем trakt ID в словарь пока он доступен из исходной карточки
            var tid = card.ids && card.ids.trakt;
            if (tid) saveTraktId(id, tid);
            log('Enriched', { id: id, title: enriched.title || enriched.name });
            _ownOps.add(id);
            Lampa.Favorite.add(folder, enriched);
            // Для look слушатель не срабатывает — удаляем id из _ownOps вручную
            // чтобы не заблокировать последующий ручной перенос в thrown
            if (folder === 'look') _ownOps.delete(id);
            if (onDone) onDone();
        }, function () {
    ...
    function initFavoriteListener() {
        ...
        Lampa.Favorite.listener.follow('remove', function (e) {
            ...
            // Lampa удаляет сериал из look раньше чем добавляет в thrown —
            // перехватываем здесь чтобы отправить drop в Trakt
            if (folder === 'look') {
                if (id && _ownOps.has(id)) {
                    // Наша собственная операция из синхронизации — пропускаем
                    _ownOps.delete(id);
                    log('Skip own remove', { folder: folder, id: id });
                    return;
                }
                if (!isEnabled()) return;
                log('User removed from look, dropping from upnext', { id: id });
                dropShowFromUpnext(e.card);
                return;
            }
