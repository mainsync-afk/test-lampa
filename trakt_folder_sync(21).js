(function () {
    ...
    function onFavoriteChanged(folder, method, card) {
        ...
        if (folder === 'book') {
            ...
            if (method === 'add') {
                // Только отправляем в Trakt — добавить в Брошено и удалить из Watchlist
                // Удаление из нативных Закладок произойдёт само при синхронизации Trakt->Lampa
                // Блокируем синхронизацию thrown пока Trakt API не обновится
                _syncBlockedUntil['thrown'] = Date.now() + 15000;
                api.addToList({ listId: listId, item: params })
                    .then(function () {
                        log('Added to thrown list');
                        // После успеха ещё 5 секунд даём Trakt обновить кэш
                        setTimeout(function () { _syncBlockedUntil['thrown'] = 0; }, 5000);
                    })
                    .catch(function (e) {
                        log('addToList error', e);
                        _syncBlockedUntil['thrown'] = 0;
                    });
                api.removeFromWatchlist(params)
    ...
    var _syncBlockedUntil = { book: 0, thrown: 0 }; // блокировка синхронизации после локального изменения
    ...
    function syncTraktToLampa(folder) {
        ...
        if (_syncBlockedUntil[folder] && Date.now() < _syncBlockedUntil[folder]) {
            log('Sync blocked for', folder);
            return;
        }
