    ...
    function onAdd(folder, card) {
        ...
        if (folder === 'book') {
            ...
            api.addToList({ listId: listId, item: params })
               .then(function () {
                   log('addToList ok');
                   setTimeout(function () { _ownOps.delete('add:' + id); }, 60000);
               })
               .catch(function (e) {
                   log('addToList err', e);
                   if (id) _ownOps.delete('add:' + id);
               });
        }
    }
    ...
    function fetchTraktItems(api, folder) {
        if (folder === 'book') {
            return fetchPages(function (p) {
                return api.watchlist({ page: p, limit: 100, mediaType: 'movies,shows', forceFresh: true });
            });
        }
        if (folder === 'thrown') {
            var listId = Lampa.Storage.get(STORAGE_LIST_ID);
            ...
            return fetchPages(function (p) {
                return api.myListItems({ listId: listId, page: p, limit: 100, forceFresh: true });
            });
        }
