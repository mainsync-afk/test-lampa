(function () {
    ...
    // look   (Смотрю)   <--> Trakt upnext (только чтение из Trakt)
    // -----------------------------------------------------------------------
    ...
    // Прокси API из плагина trakt (тот же, что использует lampa_trakt.js)
    ...
    function traktId(card) {
        if (!card) return null;
        return (card.ids && card.ids.trakt) ? String(card.ids.trakt) : null;
    }
    ...
    // Используется для операций которых нет в lampa_trakt.js (hidden/dropped)
    // -----------------------------------------------------------------------
    ...
    function traktApiRequest(method, endpoint, body) {
        ...
        return fetch(url, opts).then(function (r) {
            ...
            // 201/204 могут не возвращать body
            return r.status === 204 ? {} : r.json().catch(function () { return {}; });
        });
    }

    // -----------------------------------------------------------------------
    // Drop show — скрывает сериал из Trakt upnext/continue watching
    // POST /users/hidden/dropped {"shows": [{"ids": {"trakt": 123}}]}
    // Аналог кнопки "Drop show" в интерфейсе Trakt
    // Прогресс и история НЕ удаляются
    // -----------------------------------------------------------------------

    function dropShowFromUpnext(card) {
        var tid = traktId(card);
        if (!tid) {
            log('dropShowFromUpnext: no trakt id, skip', card);
            return Promise.resolve();
        }

        var body = { shows: [{ ids: { trakt: parseInt(tid, 10) } }] };
        log('dropShowFromUpnext', { traktId: tid });

        return traktApiRequest('POST', '/users/hidden/dropped', body)
            .then(function (res) {
                log('dropShowFromUpnext ok', res);
            })
            .catch(function (e) {
                log('dropShowFromUpnext err', e);
            });
    }
    ...
    function onAdd(folder, card) {
        ...
        if (folder === 'book') {
            ...
            if (id) {
                ...
                if (inLook) {
                    // Помечаем как собственную операцию чтобы не отправить remove в Trakt
                    _ownOps.add(id);
                    Lampa.Favorite.remove('look', inLook);
                    log('Removed from look on thrown add', id);

                    // Сообщаем Trakt что сериал брошен — убираем из upnext/continue watching
                    // Это аналог кнопки "Drop show" в Trakt без сброса истории
    ...
    function applySync(folder, traktItems) {
        ...
        // Для папки look: пропускаем если сериал уже в thrown
        // (он может оставаться в upnext пока Trakt не обработал hidden/dropped)
        var thrownItems = folder === 'look'
        ...
        var toAdd = traktItems.filter(function (c) {
            ...
            // Защита от возврата в look сериалов которые пользователь уже бросил:
            // upnext в Trakt обновляется не мгновенно после hidden/dropped
            if (thrownItems && thrownItems.has(id)) {
