(function () {
    ...
    // Trakt ID: сначала ищем в ids.trakt, потом в сохранённом поле trakt_id
    function getTraktId(card) {
        if (!card) return null;
        var tid = (card.ids && card.ids.trakt) || card.trakt_id;
    ...
    function dropShowFromUpnext(card) {
        ...
        if (!tid) {
            log('dropShowFromUpnext: no trakt id, skip', { title: card && (card.title || card.name), trakt_id: card && card.trakt_id, ids: card && card.ids });
    ...
    function applySync(folder, traktItems) {
        ...
        // Для look: пропускаем если уже в thrown (upnext может ещё не обновиться после drop)
    ...
    // -----------------------------------------------------------------------
    // Обогащение карточки через TMDB + сохранение trakt_id
    // -----------------------------------------------------------------------
    ...
    function enrichAndAdd(folder, card, onDone) {
        ...
        // Сохраняем trakt ID до обогащения — он есть в карточке из upnext/watchlist
        // но может потеряться после Object.assign с данными TMDB
        var savedTraktId = getTraktId(card);
        ...
        network.silent(url, function (data) {
            var enriched = Object.assign({}, card, data, {
                ids:      card.ids,
                // Явно сохраняем trakt_id на верхнем уровне — Lampa сохранит его
                // вместе с карточкой, и мы сможем достать при dropShowFromUpnext
                trakt_id: savedTraktId || (card.ids && card.ids.trakt) || null,
                method:   card.method || (type === 'tv' ? 'tv' : 'movie'),
                id:       id
            });
            if (type === 'tv' && enriched.name) enriched.title = enriched.name;
            log('Enriched', { id: id, title: enriched.title || enriched.name, trakt_id: enriched.trakt_id });
            _ownOps.add(id);
            Lampa.Favorite.add(folder, enriched);
            if (onDone) onDone();
        }, function () {
            log('TMDB enrich failed', id);
            // Даже без обогащения сохраняем trakt_id
            var fallback = Object.assign({}, card, {
                trakt_id: savedTraktId || null
            });
            _ownOps.add(id);
            Lampa.Favorite.add(folder, fallback);
            if (onDone) onDone();
        });
    }
